      ...
      <v-menu>
        <template v-slot:activator="{ props }">
          <v-btn
            ...
            v-bind="props"
            >{{ $t("RecentChats") }}<ChevronDownIcon size="18" class="ml-2" />
          </v-btn>
        </template>
  ...
  <perfect-scrollbar class="lgScroll chatlist">
    <v-list class="theme-list">
      ...
      >
        ...
        <!---Name-->
        <v-list-item-title
          class="text-subtitle-1 textPrimary w-100 font-weight-semibold poppins"
          >{{ chat.name }}</v-list-item-title
        >
        <!---Subtitle-->
        <v-sheet
          v-if="chat.chatHistory.slice(-1)[0].type == 'img'"
          class="bg-transparent"
        >
          <small class="textSecondary text-subtitle-2">Sent a Photo</small>
        </v-sheet>
        <div
          class="text-subtitle-2 textSecondary mt-1 text-truncate w-100"
          v-else
        >
